...
本文档详细分析 iOS "查找" (Find My) 应用中"联系人/人" (People) 模块的交互逻辑，重点在于**位置共享**的建立与管理，并结合 Firebase 提出 Android 端的实现方案。

## 1. iOS 联系人添加逻辑分析

iOS 的"联系人"功能核心在于**基于许可的双向或单向位置共享**。它不是简单的"添加好友"，而是"我把我的位置共享给你"。
...
*   当 User A 分享给 User B (Accepted) 时，云函数 (Cloud Function) 或客户端逻辑将 User B 的 UID 添加到 User A 的**主设备** (如手机) 的 `sharedWith` 字段中。
*   **Device Document**:
    ```json
    {
      "ownerId": "userA_uid",
