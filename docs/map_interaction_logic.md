...
本文档旨在描述 Android 版 "查找" (Find My) 应用的地图交互逻辑。目标是在 Android 平台上，利用 Google
Maps SDK 复刻 iOS 原生 "查找" 应用的流畅体验，同时符合 Android (API 36) 的设计规范。
...
* **地图 Padding**：当地图底部的面板向上滑动时，地图的“有效显示区域”随之改变。
    ...
    * 效果：保持 Google Logo 和定位蓝点不被面板遮挡，且 `animateCamera` 居中时会自动考虑
      Padding，使目标点位于可见区域中心。
